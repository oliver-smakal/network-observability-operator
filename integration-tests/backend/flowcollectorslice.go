...
func (flowSlice FlowcollectorSlice) CreateFlowcollectorSlice(oc *exutil.CLI) {
	...
	exutil.ApplyNsResourceFromTemplate(oc, flowSlice.Namespace, parameters...)
}
...
func (flowSlice *FlowcollectorSlice) WaitForFlowcollectorSliceReady(oc *exutil.CLI) {
	...
	exutil.AssertWaitPollNoErr(err, fmt.Sprintf("FlowCollectorSlice %s/%s did not become Ready", flowSlice.Namespace, flowSlice.Name))
}
