...
const (
	ALLOW_RELATED NWEvents = "allow-related"
	DROP          NWEvents = "drop"
)
...
func getIPFIXFlowRecordsFromAPI(oc *exutil.CLI, namespace, podName string) ([]FlowRecord, error) {
	...
	if err != nil {
		return flowRecords, fmt.Errorf("failed to query collector API: %v", err)
	}
	...
	if err := json.Unmarshal([]byte(output), &response); err != nil {
		return flowRecords, fmt.Errorf("failed to parse collector response: %v", err)
	}
