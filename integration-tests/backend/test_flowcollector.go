...
var _ = g.Describe("[sig-netobserv] Network_Observability", func() {
	...
	g.It("Author:aramesha-NonPreRelease-Medium-72875-Verify nodeSelector and tolerations with netobserv components [Serial]", func() {
		...
		defer compat_otp.DeleteLabelFromNode(oc, workerNode, "test")
		compat_otp.AddLabelToNode(oc, workerNode, "netobserv-agent", "true")
	...
	g.Context("with Loki", func() {
		...
		g.It("Author:aramesha-NonPreRelease-High-62989-Verify SCTP, ICMP, ICMPv6 traffic is observed [Disruptive]", func() {
			...
			compat_otp.SetNamespacePrivileged(oc, SCTPns)
		...
		g.It("Author:aramesha-NonPreRelease-High-69218-High-71291-Verify cluster ID and zone in multiCluster deployment [Serial]", func() {
			...
			for _, r := range zoneFlowRecords {
				expectedSrcK8SZone, err := compat_otp.GetResourceSpecificLabelValue(oc, "node/"+r.Flowlog.SrcK8S_HostName, "", `topology.kubernetes.io/zone`)
				o.Expect(err).NotTo(o.HaveOccurred())
				o.Expect(r.Flowlog.SrcK8S_Zone).To(o.Equal(expectedSrcK8SZone))

				expectedDstK8SZone, err := compat_otp.GetResourceSpecificLabelValue(oc, "node/"+r.Flowlog.DstK8S_HostName, "", `topology.kubernetes.io/zone`)
				o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.It("Author:kapjain-NonPreRelease-Longduration-High-85953-Verify FlowCollector Service deployment model [Serial][Slow]", func() {
			...
			FLPpodNoTLS, err := compat_otp.GetAllPodsWithLabel(oc, flow.Namespace, "app=flowlogs-pipeline")
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			agentPodNoTLS, err := compat_otp.GetAllPodsWithLabel(oc, flow.Namespace+"-privileged", "app=netobserv-ebpf-agent")
			o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.It("Author:kapjain-Medium-86372-Verify Gateway API three-level owner metadata [Serial]", func() {
			...
			lokilabels := Lokilabels{
				SrcK8S_Namespace: "netobserv-gateway-test",
			}
