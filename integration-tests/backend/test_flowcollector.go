...
var _ = g.Describe("[sig-netobserv] Network_Observability", func() {
	...
	g.It("Author:memodi-Medium-63185-Verify NetOberv must-gather plugin [Serial]", func() {
		...
		output, _ := oc.AsAdmin().WithoutNamespace().Run("adm").Args("must-gather", "--image", mustgatherImage, "--dest-dir="+mustgatherDir).Output()
		fmt.Print(output)

		o.Expect(output).NotTo(o.ContainSubstring("error"))

		g.By("Wait for a min before checking logs")
		time.Sleep(60 * time.Second)

		// Update directory path for log verification
		mustgatherLogsDir := mustgatherDir + "/quay-io-netobserv-must-gather-*"
	...
	g.Context("with Loki", func() {
		...
		g.It("Author:memodi-High-53595-High-49107-High-45304-High-54929-High-54840-High-68310-Verify flow correctness and metrics [Serial]", func() {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			exutil.AssertAllPodsToBeReady(oc, testServer.ServerNS)
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			exutil.AssertAllPodsToBeReady(oc, testClient.ClientNS)
			...
			lokilabels := Lokilabels{
				...
				SrcK8S_Namespace: testServer.ServerNS,
				DstK8S_Namespace: testClient.ClientNS,
		...
		g.It("Author:aramesha-NonPreRelease-High-69218-High-71291-Verify cluster ID and zone in multiCluster deployment [Serial]", func() {
			...
			for _, r := range zoneFlowRecords {
				expectedSrcK8SZone, err := exutil.GetResourceSpecificLabelValue(oc, "node/"+r.Flowlog.SrcK8S_HostName, "", `topology.kubernetes.io/zone`)
				o.Expect(err).NotTo(o.HaveOccurred())
				o.Expect(r.Flowlog.SrcK8S_Zone).To(o.Equal(expectedSrcK8SZone))

				expectedDstK8SZone, err := exutil.GetResourceSpecificLabelValue(oc, "node/"+r.Flowlog.DstK8S_HostName, "", `topology.kubernetes.io/zone`)
				o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.It("Author:aramesha-High-81677-Validate UDN with NetObserv [Serial]", func() {
			var (
				namespace        = oc.Namespace()
				networkingUDNDir = exutil.FixturePath("testdata", "networking", "udn")
			...
			for i, ns := range cudnNS {
				...
				exutil.AssertAllPodsToBeReady(oc, CUDNpods[i].namespace)
			}
			...
			exutil.AssertAllPodsToBeReady(oc, udnNS)
			...
			lokilabels := Lokilabels{
				App:              "netobserv-flowcollector",
				DstK8S_Namespace: cudnNS[1],
				DstK8S_OwnerName: CUDNpods[1].name,
				SrcK8S_Namespace: cudnNS[0],
				SrcK8S_OwnerName: CUDNpods[0].name,
			}
			...
			for _, r := range flowRecords {
				...
				o.Expect(r.Flowlog.DstK8S_NetworkName).Should(o.ContainSubstring(cudnName))
				o.Expect(r.Flowlog.SrcK8S_NetworkName).Should(o.ContainSubstring(cudnName))
			}

			g.By("Verify UDN flows")
			lokilabels = Lokilabels{
				App:              "netobserv-flowcollector",
				DstK8S_Namespace: udnNS,
				DstK8S_OwnerName: UDNpods[1].name,
				SrcK8S_Namespace: udnNS,
				SrcK8S_OwnerName: UDNpods[0].name,
			}
			...
			for _, r := range flowRecords {
				...
				o.Expect(r.Flowlog.DstK8S_NetworkName).Should(o.ContainSubstring(fmt.Sprintf("%s/%s", udnNS, udnName)))
				o.Expect(r.Flowlog.SrcK8S_NetworkName).Should(o.ContainSubstring(fmt.Sprintf("%s/%s", udnNS, udnName)))
			}
		})
		...
		g.It("Author:aramesha-High-83022-Validate CUDN with Localnet [Serial]", func() {
			...
			workerNode, getNodeErr := exutil.GetFirstWorkerNode(oc)
			o.Expect(getNodeErr).NotTo(o.HaveOccurred())
			o.Expect(workerNode).NotTo(o.BeEmpty())

			exutil.By("Create NMState CR")
			...
			exutil.By("Configure NNCP for creating OvnMapping NMstate Feature")
			...
			defer func() {
				ovnmapping, deferErr := exutil.DebugNodeWithChroot(oc, workerNode, "ovs-vsctl", "get", "Open_vSwitch", ".", "external_ids:ovn-bridge-mappings")
				...
				if strings.Contains(ovnmapping, ovnMappingPolicy.localnet1) {
					...
					_, err := exutil.DebugNodeWithChroot(oc, workerNode, "ovs-vsctl", "set", "Open_vSwitch", ".", "external_ids:ovn-bridge-mappings=\"physnet:br-ex\"")
					o.Expect(err).NotTo(o.HaveOccurred())
				}
			}()
			...
			exutil.By("Create two namespaces and label them")
			for _, ns := range cudnNS {
			...
			exutil.By("Create secondary localnet CUDN")
			...
			exutil.By("Deploy statefulset in both cudnNS")
			for _, ns := range cudnNS {
				defer removeResource(oc, true, true, "statefulset", "hello", "-n", ns)
				exutil.ApplyNsResourceFromTemplate(oc, ns, "-f", udnStatefulSetTemplate, "NETWORK_NAME="+secondaryCUDNName)
			...
			g.By("Validate connection among CUDN pods")
			cudn1Pods, err := exutil.GetAllPods(oc, cudnNS[0])
			o.Expect(err).NotTo(o.HaveOccurred())
			cudn2Pods, err := exutil.GetAllPods(oc, cudnNS[1])
			o.Expect(err).NotTo(o.HaveOccurred())
			startTime := time.Now()
			...
			lokilabels := Lokilabels{
				App:              "netobserv-flowcollector",
				DstK8S_Namespace: cudnNS[1],
				DstK8S_OwnerName: "hello",
				SrcK8S_Namespace: cudnNS[0],
				SrcK8S_OwnerName: "hello",
			}
			...
			for _, r := range flowRecords {
				...
				o.Expect(r.Flowlog.DstK8S_NetworkName).Should(o.ContainSubstring(secondaryCUDNName))
				o.Expect(r.Flowlog.SrcK8S_NetworkName).Should(o.ContainSubstring(secondaryCUDNName))
			}
		})
		...
		g.It("Author:aramesha-NonPreRelease-Longduration-Medium-81410-NetObserv with eBPF manager [Serial][Slow]", func() {
			...
			bpfDir := exutil.FixturePath("testdata", "netobserv", "bpfman")
		...
		g.It("Author:memodi-NonPreRelease-High-82637-Verify IPSec feature [Disruptive]", func() {
			exutil.By("Check if IPSec is enabled in the cluster")
			...
			o.Expect(popMetricValue(metrics)).Should(o.BeNumerically(">", 0))
			o.Expect(err).NotTo(o.HaveOccurred())
			verifyEBPFFeatureMetrics(oc, "IPSec")
		})
		...
		g.It("Author:kapjain-NonPreRelease-Longduration-High-85953-Verify FlowCollector Service deployment model [Serial][Slow]", func() {
			...
			FLPpods, err := exutil.GetAllPodsWithLabel(oc, flow.Namespace, "app=flowlogs-pipeline")
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			agentPods, err := exutil.GetAllPodsWithLabel(oc, flow.Namespace+"-privileged", "app=netobserv-ebpf-agent")
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			WaitForDeploymentPodsToBeReady(oc, flow.Namespace, "flowlogs-pipeline")
			result = verifyDeploymentReplicas(oc, "flowlogs-pipeline", flow.Namespace, 3, "")
			...
			hpaFile := exutil.ProcessTemplate(oc, "--ignore-unknown-parameters=true", "-f", hpaYAML, "-p", "NAMESPACE="+namespace)
			...
			result = verifyDeploymentReplicas(oc, "flowlogs-pipeline", flow.Namespace, 4, ">")
			o.Expect(result).To(o.BeTrue(), "HPA should scale deployment above 5 replicas when unmanagedReplicas is true")
			...
			g.By("Patch FlowCollector to disable TLS by setting SERVER_NOTLS=true")
			err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("flowcollector", "cluster", "--type=merge", "-p", `{"spec":{"processor":{"advanced":{"env":{"SERVER_NOTLS":"true"}}}}}`).Execute()
			o.Expect(err).NotTo(o.HaveOccurred())

			g.By("Wait for FLP pods to restart with new configuration")
			time.Sleep(30 * time.Second)
			waitUntilDeploymentReady(oc, "flowlogs-pipeline", flow.Namespace)

			g.By("Verify FLP logs show 'Starting GRPC server - no TLS'")
			FLPpodNoTLS, err := exutil.GetAllPodsWithLabel(oc, flow.Namespace, "app=flowlogs-pipeline")
			o.Expect(err).NotTo(o.HaveOccurred())

			g.By("Get FLP pod logs to check GRPC server startup message (no TLS)")
			flpLogsNoTLS, err := oc.AsAdmin().WithoutNamespace().Run("logs").Args("-n", flow.Namespace, FLPpodNoTLS[0], "--tail=100").Output()
			o.Expect(err).NotTo(o.HaveOccurred())
			o.Expect(flpLogsNoTLS).To(o.ContainSubstring("Starting GRPC server - no TLS"), "FLP logs should contain 'Starting GRPC server - no TLS'")

			g.By("Verify agent logs show 'Starting GRPC client - no TLS'")
			agentPodNoTLS, err := exutil.GetAllPodsWithLabel(oc, flow.Namespace+"-privileged", "app=netobserv-ebpf-agent")
			o.Expect(err).NotTo(o.HaveOccurred())

			g.By("Get agent pod logs to check GRPC client startup message (no TLS)")
			agentLogsNoTLS, err := oc.AsAdmin().WithoutNamespace().Run("logs").Args("-n", flow.Namespace+"-privileged", agentPodNoTLS[0], "--tail=100").Output()
			o.Expect(err).NotTo(o.HaveOccurred())

			o.Expect(agentLogsNoTLS).To(o.ContainSubstring("Starting GRPC client - no TLS"), "Agent logs should contain 'Starting GRPC client - no TLS'")

			// Verify logs
			g.By("Wait for a min before logs gets collected and written to loki in Non TLS mode")
			startTime = time.Now()
			time.Sleep(60 * time.Second)
		...
		g.Context("with Kafka", func() {
			...
			g.It("Author:aramesha-NonPreRelease-Longduration-Critical-56362-High-53597-High-56326-Verify network flows are captured with Kafka with TLS [Serial][Slow]", func() {
			...
			g.It("Author:aramesha-NonPreRelease-Longduration-High-57397-High-65116-Verify network-flows export with Kafka and netobserv installation without Loki[Serial]", func() {
				...
				podLogs, err := exutil.WaitAndGetSpecificPodLogs(oc, consumer.Namespace, "", consumerPodName, `'{"AgentIP":'`)
			...
			g.It("Author:aramesha-NonPreRelease-High-64880-High-75340-Verify secrets copied for Loki and Kafka when deployed in NS other than flowcollector pods and networkPolicy enabled[Serial]", func() {
				g.Skip("Skipping this test case because Manual loki mode config is currently not functional")
				g.By("Deploy FlowCollector with Kafka and networkPolicy enabled")
				lokiURL := fmt.Sprintf("https://%s-gateway-http.%s.svc.cluster.local:8080/api/logs/v1/network/", ls.Name, lokiStackNS)

				allowAdditionalNamespaces := fmt.Sprintf("\"%s\", %s", lokiStackNS, additionalNamespaces)
				...
				flow := Flowcollector{
					...
					KafkaTLSEnable:                    "true",
					KafkaNamespace:                    kafkaNs,
					NetworkPolicyAdditionalNamespaces: []string{allowAdditionalNamespaces},
				}

				defer flow.DeleteFlowcollector(oc)
				flow.CreateFlowcollector(oc)

				g.By(fmt.Sprintf("Verify networkPolicy is deployed with %s", kafkaNs))
				ingressPolicy, err := oc.AsAdmin().Run("get").Args("networkPolicy", "netobserv", "-n", flow.Namespace, "-o", "jsonpath='{.spec.ingress[-1].from[0].namespaceSelector.matchLabels}'").Output()
				o.Expect(err).ToNot(o.HaveOccurred())
				o.Expect(ingressPolicy).To(o.ContainSubstring(kafkaNs))

				egressPolicy, err := oc.AsAdmin().Run("get").Args("networkPolicy", "netobserv", "-n", flow.Namespace, "-o", "jsonpath='{.spec.egress[-1].to[0].namespaceSelector.matchLabels}'").Output()
				o.Expect(err).ToNot(o.HaveOccurred())
				o.Expect(egressPolicy).To(o.ContainSubstring(kafkaNs))
		...
		g.Context("with VMs", func() {
			...
			g.It("Author:aramesha-NonPreRelease-Longduration-High-76537-Verify flow enrichment for VM's secondary interfaces [Disruptive][Slow]", func() {
				...
				// wait for VM to get IP assigned
				time.Sleep(60 * time.Second)

				vm1Ip, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("vmi", testVM1.Name, "-n", testVM1.Namespace, "-ojsonpath='{.status.interfaces[1].ipAddress}'").Output()
				o.Expect(err).ToNot(o.HaveOccurred())
				o.Expect(vm1Ip).NotTo(o.BeEmpty())
				...
				flow := Flowcollector{
					...
					LokiNamespace:     lokiStackNS,
					EBPFPrivileged:    "true",
					SecondaryNetworks: secNetConfig,
				}
				...
				secondaryNetworkName, err := oc.AsAdmin().Run("get").Args("flowcollector", "cluster", "-o", "jsonpath='{.spec.processor.advanced.secondaryNetworks[0].name}'").Output()
				o.Expect(err).ToNot(o.HaveOccurred())
				...
				lokilabels := Lokilabels{
					...
					SrcK8S_OwnerName: testVM2.Name,
					DstK8S_Namespace: testNS,
					DstK8S_OwnerName: testVM1.Name,
				}
				...
				vm1PodName, err := exutil.GetAllPodsWithLabel(oc, testNS, "vm.kubevirt.io/name="+testVM1.Name)
				o.Expect(err).NotTo(o.HaveOccurred())
				...
				vm2PodName, err := exutil.GetAllPodsWithLabel(oc, testNS, "vm.kubevirt.io/name="+testVM2.Name)
				o.Expect(err).NotTo(o.HaveOccurred())
			...
			g.It("Author:aramesha-NonPreRelease-Longduration-Medium-85887-Verify UDN with VMs [Disruptive][Slow]", func() {
				...
				vm3Ip, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("vmi", testVM3.Name, "-n", testVM3.Namespace, "-ojsonpath='{.status.interfaces[0].ipAddress}'").Output()
				o.Expect(err).ToNot(o.HaveOccurred())
				...
				lokilabels := Lokilabels{
					App:              "netobserv-flowcollector",
					SrcK8S_Namespace: udnNS,
					SrcK8S_OwnerName: testVM4.Name,
					DstK8S_Namespace: udnNS,
					DstK8S_OwnerName: testVM3.Name,
				}
				...
				vm3podname, err := exutil.GetAllPodsWithLabel(oc, udnNS, "vm.kubevirt.io/name="+testVM3.Name)
				o.Expect(err).NotTo(o.HaveOccurred())
				o.Expect(vm3podname).NotTo(o.BeEmpty())
				// Get VM4 launcher pod name
				vm4podname, err := exutil.GetAllPodsWithLabel(oc, udnNS, "vm.kubevirt.io/name="+testVM4.Name)
				o.Expect(err).NotTo(o.HaveOccurred())
				o.Expect(vm4podname).NotTo(o.BeEmpty())

				for _, r := range flowRecords {
					o.Expect(r.Flowlog.DstK8S_Name).Should(o.ContainSubstring(vm3podname[0]))
					o.Expect(r.Flowlog.SrcK8S_Name).Should(o.ContainSubstring(vm4podname[0]))
					o.Expect(r.Flowlog.DstK8S_OwnerType).Should(o.ContainSubstring("VirtualMachineInstance"))
					o.Expect(r.Flowlog.SrcK8S_OwnerType).Should(o.ContainSubstring("VirtualMachineInstance"))
					o.Expect(r.Flowlog.Udns).Should(o.ContainElement(fmt.Sprintf("%s/%s", udnNS, udnName)))
					o.Expect(r.Flowlog.DstK8S_NetworkName).Should(o.ContainSubstring(fmt.Sprintf("%s/%s", udnNS, udnName)))
			...
			g.It("Author:aramesha-High-85935-Validate CUDN with Localnet and VM's[Serial]", func() {
				var (
					...
					opNamespace       = "openshift-nmstate"
					nmStateDir        = exutil.FixturePath("testdata", "networking", "nmstate")
				...
				vm5Ip, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("vmi", testVM5.Name, "-n", testVM5.Namespace, "-ojsonpath='{.status.interfaces[1].ipAddress}'").Output()
				o.Expect(err).ToNot(o.HaveOccurred())
				...
				lokilabels := Lokilabels{
					App:              "netobserv-flowcollector",
					SrcK8S_Namespace: cudnNS[1],
					SrcK8S_OwnerName: testVM6.Name,
					DstK8S_Namespace: cudnNS[0],
					DstK8S_OwnerName: testVM5.Name,
				}
			...
			//Add future NetObserv + Loki + VM test-cases here
		})
	})
})
