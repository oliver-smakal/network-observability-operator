...
func (a AzureBSInfoProvider) GetIPs(oc *exutil.CLI) (*Ips, error) {
	...
	resourceGroupName, err := exutil.GetAzureCredentialFromCluster(oc.AsAdmin())
	if err != nil {
		e2e.Logf("Error reading cluster's azure credentials")
		return nil, err
	}

	azSession, err := exutil.NewAzureSessionFromEnv()
	if err != nil {
		e2e.Logf("Error creating a new azure session")
		return nil, err
	}

	bootstrapInstanceID, err := exutil.GetAzureVMInstance(azSession, bootstrapName, resourceGroupName)
	if err != nil {
	...
	state, err := exutil.GetAzureVMInstanceState(azSession, bootstrapInstanceID, resourceGroupName)
	if err != nil {
	...
	bootstrapPublicIP, err := exutil.GetAzureVMPublicIPByNameRegex(azSession, resourceGroupName, bootstrapInstanceID)
	if err != nil {
	...
	bootstrapPrivateIP, err := exutil.GetAzureVMPrivateIP(azSession, resourceGroupName, bootstrapInstanceID)
	if err != nil {
