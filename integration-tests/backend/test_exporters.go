...
var _ = g.Describe("[sig-netobserv] Network_Observability", func() {
	...
	g.It("Author:aramesha-High-64156-Verify IPFIX-exporter [Serial]", func() {
		...
		compat_otp.SetNamespacePrivileged(oc, IPFIXns)
		...
		_, err = compat_otp.WaitAndGetSpecificPodLogs(oc, IPFIXns, "", FLPconsumerPod, `"Type:IPFIX"`)
		compat_otp.AssertWaitPollNoErr(err, "Did not find Type IPFIX in ipfix-collector pod logs")
	})
