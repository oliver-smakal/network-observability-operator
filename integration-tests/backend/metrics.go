...
func verifyEBPFFeatureMetrics(oc *exutil.CLI, feature string) {
	...
	case "Drops":
		...
		o.Expect(metrics).Should(o.BeNumerically("~", 4, 12), "Drop metrics are beyond threshold values")
	case "RTT":
	...
	case "IPSec":
		...
		o.Expect(metrics).Should(o.BeNumerically("~", 50, 100), "IPSec metrics are beyond threshold values")
	}
}
