...
// verifyDeploymentReplicas verifies the deployment replica count and returns a bool
...
func verifyDeploymentReplicas(oc *exutil.CLI, deployment, namespace string, expectedValue int, operator string) bool {
	replicas, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("deployment", deployment, "-n", namespace, "-o", "jsonpath='{.status.replicas}'").Output()
	if err != nil {
		e2e.Logf("Error getting deployment %s replicas in namespace %s: %v", deployment, namespace, err)
		return false
	}
	...
	if err != nil {
		e2e.Logf("Error converting replicas '%s' to int: %v", replicas, err)
		return false
	}

	if operator == "" {
		// Exact match
		return replicasInt == expectedValue
	}
	...
	case "==":
		return replicasInt == expectedValue
	case "~":
		// Approximate equality - treat same as exact match for integers
		return replicasInt == expectedValue
	default:
