...
func CreateHealthDashboard(netobsNs, nsFlowsMetric string) (string, error) {
	...
	d.Rows = append(d.Rows, NewRow("Resource usage", true, "250px", []Panel{
		...
		NewPanel("Overall memory", metricslatest.ChartTypeStackArea, "", 6,
			NewTarget(`sum(container_memory_working_set_bytes{container="netobserv-ebpf-agent"})`, "eBPF agent"),
			NewTarget(`sum(container_memory_working_set_bytes{container="flowlogs-pipeline"})`, "flowlogs-pipeline"),
			NewTarget(`sum(container_memory_working_set_bytes{container!="",pod=~"netobserv-controller-manager.*"})`, "operator"),
		),
		...
		NewPanel("eBPF agent memory - top 10 pods", metricslatest.ChartTypeStackArea, "", 6,
			NewTarget(`topk(10, container_memory_working_set_bytes{container="netobserv-ebpf-agent"})`, "{{pod}}"),
		),
		...
		NewPanel("Flowlogs-pipeline memory - top 10 pods", metricslatest.ChartTypeStackArea, "", 6,
			NewTarget(`topk(10, container_memory_working_set_bytes{container="flowlogs-pipeline"})`, "{{pod}}"),
		),
	}))
